...
public class GradleDependencyFinderWithSubFileFinder implements DependencyFinder {
	Connection c;
	Writer out;

	public GradleDependencyFinderWithSubFileFinder(Connection c, Writer out) {
		this.c = c;
		this.out = out;
	}
	...
	// Gather additional subfiles to be parsed
	...
	private List<String> getVersionNum (String line, String file, String url) {
		...
		try {
			...
			for (String tot: rawVersions) {
				...
				if (m.find()) { 
					List<String> temp = getVersionFromVariable(file, ext, m.group(), url);
					if (!temp.isEmpty())
	...
	private ArrayList<String[]> gatherSubfiles (String originalfile, String url) {
		...
		// Find subfiles
		String newurl = CollectFileFromURL.urlGithubPlainText(url).replaceAll("/build.gradle", "");
		String settings = CollectFileFromURL.getFile(newurl + "/settings.gradle");

		// Record files and urls
		ArrayList<String[]> filestoprocess = new ArrayList<>();
		filestoprocess.add(new String[]{originalfile, newurl + "/build.gradle"});

		// Finds include commands
		Matcher minclude = INCLUDE.matcher(settings);
		while (minclude.find()) {
			String temp = minclude.group();
			Matcher mquoted = QUOTED.matcher(temp);

			// Finds quoted text within include commands... can be multiple per line
			while (mquoted.find()) {
				String extension = mquoted.group().replaceAll(":", "/").replaceAll("[\'\"]", "") + "/build.gradle";
				String tempfile = CollectFileFromURL.getFile(newurl + extension);

				// Records if file not found, otherwise saves file and adds to to-do list for processing
				if (tempfile.isEmpty()) {
					//printString(newurl+extension + " was not found");
				} else {
					// Record
					filestoprocess.add(new String[]{tempfile, newurl+extension});

					// Save file
					String folderurl = url.replace("https://github.com/", "").replace("/", "+").replace("+build.gradle", extension);
					String dir = "D://Build Scripts/gradle/" + folderurl.charAt(0) + "/" + folderurl;
					try {
						File f = new File(dir.replaceFirst("/build.gradle", ""));
						f.mkdirs();
						try (BufferedWriter out = new BufferedWriter(new FileWriter(new File(dir)))) {
							out.write(tempfile);
						} catch (Exception e) {
							e.printStackTrace();
						}
					} catch (NullPointerException e1) { 
						System.err.println(dir + " not saved");
					}
				}
			}
		}
