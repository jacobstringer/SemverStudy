...
public class ProductionParseFiles implements ProductionMBean {
	...
	private List<ProducerGetFiles> producers = new ArrayList<ProducerGetFiles>();
	...
	public void start() {
		...
		for (int i=0;i<PRODUCER_COUNT;i++) {
			ProducerGetFiles producer = new ProducerGetFiles(queue, CONSUMER_COUNT);
		...
		for (int i=0;i<CONSUMER_COUNT;i++) {
			...
			try {
				...
				//consumer = new ConsumerGetCommitInfo(queue, connections.get(connections.size()-1));
				consumer = new ConsumerGetSaveCommitJSON(queue, connections.get(connections.size()-1));
	...
	public void stop() {
		...
		for (ProducerGetFiles producer:producers) {
			producer.stop();
		}
