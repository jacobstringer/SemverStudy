package auxiliary_AntIvyCounter;
...
public class checkAnt {
	...
	private static String path = "D:\\Build Scripts\\Build\\";
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			out = new BufferedWriter(new FileWriter("data/IvyCounts.csv"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		while(true) {
			...
			try {
				in2 = new BufferedReader(
						new FileReader(path + file.charAt(0) + "\\" + file + "+build.xml"));
			} catch (FileNotFoundException e) {
				e.printStackTrace();
				continue;
			}
