...
public class ProductionParseFiles implements ProductionMBean {
	...
	private BufferedWriter outstats = null;
	...
	public void start() {
		...
		try {
			...
			outstats = new BufferedWriter(new FileWriter("StatsTests.txt"));
		...
		for (int i=0;i<CONSUMER_COUNT;i++) {
			...
			try {
				...
				consumer = new ConsumerParseFiles(queue, connections.get(connections.size()-1), out);
	...
	public void writeNotFound() {
		try {
			int notfound = 0;
			notfound = consumers.stream().map(x -> x.notfound).reduce(0, (c, e) -> c + e);
			outstats.write(Integer.toString(notfound));
			outstats.write(" variables not found after ");
			outstats.write(Integer.toString(producers.get(0).count - queue.size()));
			outstats.write(" files analysed\n");
		} catch (IOException e1) {}
	}
